...
<img src="https://github.com/leejh96/addinedu_frontend/blob/master/summary/images/img.png?raw=true"></img>

- 사용자가 사용하는 디바이스(클라이언트)와 서비스 회사의 서버 컴퓨터가 1:1로 연결되어 있다는 가상의 구성
- 클라이언트와 서버간의 통신 : Request(요청) / Response(응답) 세트로 구성된 통신
- Request : 클라이언트
  Ex) 브라우저에 주소입력 후 엔터 => 요청
- Response : 서버
  Ex) 요청을 받은 사이트의 정보나 데이터를 클라이언트에 전송 => 응답

- 클라이언트 : 사용자가 사용하는 디바이스에서 실행되는 소프트웨어
  Ex) 모바일 앱/ PC 소프트웨어/ 브라우저(웹앱/웹페이지)
- 서버 : 서비스 제공하는 컴퓨터에서 실행되는 소프트웨어
  Ex) 서버 소프트웨어 : Apache(아파치), IIS

- 웹 개발 : 클라이언트(브라우저) / 서버(Apache(Linux), IIS(window server))
...
- Front End Develop
  ...
  - 웹 개발
    - 브라우저 통해서 화면 표시
    - 프로그래밍 언어(HTML, CSS, Javascript) 해석(실행) 시점 : 브라우저에서 해석/실행
